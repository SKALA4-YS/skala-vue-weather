...
export const practiceDays = [
  {
    ...
    requirements: [
      '배열 렌더링: `v-for` 와 `:key`',
      '조건부 라벨: `v-if` / `v-else` 로 25도 분기',
      '한글 검색: `:value` 와 `@input`',
      '카드 선택 상태바와 `@click.stop` 상세보기',
      '본인만의 데이터와 Mockup 추가',
    ],
    done: [':key에 인덱스 대신 id를 바인딩해 정렬 시 DOM 재사용 문제를 피함', 'v-model 대신 :value + @input을 직접 연결하고 IME 조합을 처리', '미세먼지 등급 배지, 외출 준비물 추천, 정렬·필터를 추가'],
    demo: 'embed',
    route: '/practice/1',
  },
  {
    ...
    requirements: ['반응형 상태를 `ref` 로 관리', '검색 결과를 `computed` 로 가공', '선택 도시를 `watch` 로 감시', '검색어를 `watchEffect` 로 자동 추적', '본인만의 상태와 watcher 추가'],
    done: ['함수로 쓰던 목록 가공을 computed로 바꿔 캐싱을 적용', '다중 소스 watch와 getter로 좁힌 watch를 추가', 'Watcher 모니터링 패널을 만들어 감시 결과를 화면에서 확인'],
    demo: 'embed',
    route: '/practice/2',
  },
  {
    ...
    requirements: [
      ...
      '`BaseDashboardCard` 에 `<slot>` 배치',
      '`SearchBar` 는 props로 받고 `update-query` emit',
      '`WeatherCard` 는 `select-card` / `click-detail` emit',
      '컴포넌트마다 `<style scoped>` 분리',
    ],
    done: ['한 화면을 8개 컴포넌트로 나누고 단방향 흐름을 유지', 'fallthrough 대신 이름 붙인 이벤트로 전환', '슬롯 내용은 부모 스코프에서 컴파일된다는 점을 확인'],
    demo: 'embed',
    route: '/practice/3',
  },
  {
    ...
    requirements: [
      ...
      '`App.vue` 에 `RouterLink` 와 `RouterView` 배치',
      '상세보기의 `alert` 를 `router.push` 로 교체',
      '`/weather/:cityId` 로 지역 상세 페이지 구성',
      '본인의 추가 view 작성',
    ],
    done: ['화면 단위 지연 로딩으로 청크를 분리', '검색어를 URL 쿼리에 동기화해 새로고침·공유에도 유지', '같은 경로 규칙 간 이동은 onMounted가 실행되지 않아 params를 watch'],
    demo: 'link',
  ...
  },
  {
    ...
    title: 'Store 적용',
    topic: 'Pinia 전역 상태',
    requirements: ['`configStore` 에 state / getters / actions 구성', '단위 변경 UI를 내비게이션 옆에 배치', '메인과 상세에 단위 설정 적용', '본인만의 추가 Store 작성'],
    done: [
      'configStore는 Options 스타일, favoriteStore는 setup 스타일로 작성해 비교',
      '변환 로직을 composable로 묶어 여러 화면에서 재사용',
      '원본은 섭씨로 두고 표시할 때만 변환해 판정 기준이 깨지지 않게 함',
    ],
    demo: 'link',
    route: '/running',
  },
  {
    day: 6,
    title: '데이터 연동',
    topic: 'Axios · OpenWeatherMap',
    requirements: ['OpenWeatherMap으로 실제 날씨 데이터 적용', '제공되는 API를 추가해 기능 확장', '기타 외부 API를 추가해 기능 확장'],
    done: ['axios 인스턴스와 인터셉터로 공통 설정과 에러 메시지를 정리', '현재 날씨·대기오염·예보를 Promise.allSettled로 병렬 호출', 'Open-Meteo에서 자외선과 일출·일몰을 추가로 연동'],
    demo: 'link',
    route: '/weather/city_01',
  },
  {
    ...
    title: 'UI 라이브러리',
    topic: 'Element Plus',
    requirements: ['외부 UI Library를 선정해 적용', '실제 날씨 데이터와 추가 API로 기능 확장'],
    done: ['el-switch · el-progress · ElMessage · ElMessageBox 적용', '러닝 지수 컨셉을 만들고 감점 내역을 화면에 노출', '지역을 30개로 늘리고 목록은 배치 조회로 전환'],
    demo: 'link',
    route: '/running',
  },
  {
    day: 8,
    title: '빌드와 배포',
    topic: 'ESLint · 환경 변수 · Vercel',
    requirements: ['ESLint로 점검해 Error를 없앤다', 'API 키는 환경 변수로 두고 Git에 올리지 않는다', 'Project를 Build 한다', 'Build된 정적 파일을 호스팅한다'],
    done: ['eqeqeq 규칙을 켜고 48개 파일에서 문제 0건 확인', '.env.staging / .env.production 으로 모드별 빌드 구성', 'Vercel에 GitHub 저장소를 연결해 push마다 자동 배포'],
    demo: 'link',
