...
컨셉이 러닝인 만큼 화면도 스포츠 브랜드 사이트처럼 잡았습니다.

## 브랜드 마크

`RW` 이니셜을 러닝화 옆모습으로 바꿨습니다. 외부 아이콘 없이 SVG path로 직접 그렸고,
갑피 · 밑창 · 신발끈 · 속도선을 나눠서 색을 따로 줍니다.
앞코가 오른쪽으로 향해 달리는 방향과 맞습니다.
...
큰 글자에는 `letter-spacing`을 음수로 줘서 자간을 좁혔습니다.
반대로 `TODAY'S PICK`, `TOP RUNNERS`, `TEMP` 같은 **영문 대문자 라벨은 자간을 넓혀** 대비를 만들었습니다.
한글만 쓰면 대문자 효과가 없어서 영문 라벨을 함께 적었습니다.

## 다이내믹한 요소

- **사선 스트라이프** — 히어로 오른쪽 위에 `repeating-linear-gradient`를 `skewX`로 기울여 속도감을 줍니다
- **카드 좌측 레일** — 점수 색이 그대로 들어간 세로 바. 마우스를 올리면 4px에서 8px로 굵어집니다
- **지표 4분할** — 기온 · 습도 · 바람 · 미세먼지를 경기 기록판처럼 나란히 배치
- **CTA 버튼** — 검정 배경 알약 버튼. 마우스를 올리면 글자와 화살표 사이가 벌어집니다

```css
.cta { transition: gap 0.16s ease; }
.cta:hover { gap: 12px; }
```

색은 화이트-블루를 유지하되 점수 색(초록 · 파랑 · 주황 · 빨강)이 지도와 카드, 순위에서
같은 기준으로 쓰이게 맞췄습니다.
